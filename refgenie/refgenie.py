...
def copy_or_download_file(input_string, outfolder):
    ...
    parts = (
        ["wget -O", result_file, input_string]
        if is_url(input_string)
        else ["cp", input_string, result_file]
    )
...
def refgenie_build(gencfg, genome, asset_list, recipe_name, args):
    ...
    if args.config_file and not os.path.isfile(args.config_file):
        _LOGGER.debug("Config file path isn't a file: {}".format(args.config_file))
...
def get_dir_digest(path, pm=None):
    ...
    if not is_command_callable("md5sum"):
        raise OSError(
            "md5sum command line tool is required for asset digest calculation. \n"
...
def _raise_missing_recipe_error(recipe):
    ...
    raise MissingRecipeError(
        "Recipe '{}' not found. Available recipes: {}".format(
            recipe, ", ".join(list(asset_build_packages.keys()))
        )
    )
