...
```python
def get_asset(self, genome_name, asset_name, strict_exists=True, check_exist=<function RefGenConf.<lambda> at 0x7f353fdc51e0>)
```
...
```python
def list_remote(self, get_url=<function RefGenConf.<lambda> at 0x7f353fdc5400>)
```
...
```python
def pull_asset(self, genome, assets, genome_config, unpack=True, get_json_url=<function RefGenConf.<lambda> at 0x7f353fdc5510>, get_main_url=None)
```
